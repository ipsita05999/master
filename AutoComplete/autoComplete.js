...
function autoComplete(inp, arr) {
  inp.addEventListener("input", () => {
    ...
    inp.parentElement.appendChild(div1);
    for (let i = 0; i < arr.length; i++) {
        if(arr[i].substr(0,value.length).toUpperCase() === value.toUpperCase()){
            let div = document.createElement("div");
            div.innerHTML= '<strong>'+arr[i].substr(0,value.length)+'</strong>';
            div.innerHTML+=arr[i].substr(value.length);
            div.innerHTML+="<input type='hidden' value='"+arr[i]+"'>";
            div.addEventListener('click', ()=>{
                inp.value= div.getElementsByTagName('input')[0].value;
                closeAllLists();
            });
